...
"""
採点侍 WebUI 版 (pywebview) — PyInstaller spec ファイル

ビルド方法:
  pyinstaller saitensamurai_webui.spec --noconfirm --clean

Windows では pywebview が WebView2 (Edge) を使う。WebView2 ランタイムは
Windows 10/11 に標準搭載のため追加インストールは不要。
pywebview の platform モジュールは pyinstaller-hooks-contrib の
hook-webview が収集するが、確実を期して hiddenimports にも明示する。
"""
...
EXE_NAME = f'SaitenSamurai_WebUI_v{_version}'

WEBUI = os.path.join(os.path.dirname(os.path.abspath(SPEC)), 'webui')

a = Analysis(
    ['webui/app.py'],
    pathex=[MAIN_SRC, WEBUI],
    binaries=[] + _sklearn_binaries + _joblib_binaries,
    datas=[
        ('resources/icon.ico', 'resources'),
        ('resources/samurai.png', 'resources'),
        ('webui/static', 'static'),          # HTML/CSS/JS 一式（app.py が _MEIPASS/static を参照）
    ] + _sklearn_datas + _joblib_datas,
    hiddenimports=[
        # webui ブリッジ（bridge 内の遅延 import も含めて明示）
        'api',
        'api.bridge',
        'api.jobs',
        'api.checker',
    ...
    excludes=[
        ...
        # --- 不要な標準ライブラリ ---
        # 注: tk 版 spec と異なり 'socketserver' は除外しない
        #     （pywebview の webview/http.py が必要とする — smoke test で検知済み）
        'sqlite3',
        'xmlrpc',
        'ftplib',
        'imaplib',
