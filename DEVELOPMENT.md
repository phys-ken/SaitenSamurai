...
- **answer_key.xlsx**: 問題番号列に範囲表記「1-3」を書くと、連続する3つのマーク行を
  ...
  なお `load_template` は `pd.read_excel(..., dtype={'正答': str})` で読みます。
  dtype を指定しないと pandas が正答列を型推論して `'024'` を `24` にしてしまい、
  文字数＝消費行数が狂って設問全体が黙って誤採点になります。型推論は列単位なので、
  正答列に英字が1つでも混ざると起きません（＝answer_key の他の行しだいで再現します）。
  問題番号列に dtype を指定してはいけません。`_check_excel_date_cell` が
  `isinstance(値, datetime)` で Excel の日付自動変換を検出しているためです。
...
- **自動割付**: 単独表記の行に複数文字の正答を書くと、正答の文字数ぶん連続行を
  ...
  別の登録があれば重複エラーになります。特例「全員正解」で正答が空欄の場合は
  消費行数を推論できないため範囲表記を推奨します。単独表記でもエラーにはせず
  1行ぶんとして扱いますが（1行の設問を全員正解にする場合があるため）、
  `answer_key_checker` が警告を出します。
...
- **チェック内容**: `load_template` のバリデーション（範囲重複・範囲長≠正答文字数・
  ...
  さらに、末尾の取りこぼし警告（座標定義より手前で answer_key が終わっている場合。
  中抜け警告は最初と最後の使用行の間しか見ないため、最後の問の登録漏れを拾えない）と、
  特例「全員正解」で正答空欄・単独表記の警告。
- **警告の文面**: この検査は登録ミスに気づける唯一の入口なので、
  「問題ありません」のような読み流しを促す表現を入れないこと。
  代わりに典型的な原因（範囲表記の書き忘れ、正答の入れ忘れ）を書く。
...
フォントのパスをモジュールに直接書かず、`constants.find_japanese_font()` を使ってください。
候補（MS ゴシック → メイリオ → 游ゴシック → MS 明朝 → Noto Sans CJK → DejaVu）を
順に探し、見つからなければ警告ログを1回だけ出して `None` を返します。

フォント取得の失敗は例外にならず、**黙って読めない出力になる**のが厄介な点です。
Pillow は `ImageFont.load_default()` に落ちてサイズ指定が効かない極小ビットマップになり、
reportlab は Helvetica に落ちて日本語が出ず、matplotlib は豆腐（□□□）になります。
いずれも採点結果としては壊れているのに、処理は正常終了します。
新しく日本語を描く箇所を足すときは、失敗時に必ず警告を出してください。

### 失敗を黙らせない

このソフトは非エンジニアの教員が使い、出力が壊れると生徒の成績に直結します。
**「例外は出ないが結果が間違っている」経路を作らないこと。** 具体的には:

- 入力の解釈に失敗したらエラーで止める（採点を続けない）
- 止めるほどでない異常は警告として残し、文面に「問題ありません」のような
  読み流しを促す表現を入れない
- フォールバックを書くときは、フォールバックしたこと自体をログに出す
